...
def get_batch_sdf_jacobian(sdf, x, out_dim=1):
    """
    :param decoder: DeepSDF Decoder
    :param lat_vec: torch.Tensor (code_len,), latent code
    ...
    """
    n = x.shape[0]
    input_x = x.clone().detach() # (n,3)
    # latent_repeat = lat_vec.expand(n, -1) # (n, code_len)
    ...
    # # Clear gradients and free up GPU memory
    # sdf.zero_grad()
    # input_x.grad = None
...
def get_robust_res(res, b):
    ...
    """
    # print(res.shape[0])
    res = res.view(-1, 1, 1) # to shape ( x ,1, 1)
